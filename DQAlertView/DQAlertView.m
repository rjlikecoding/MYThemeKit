...
- (void)dismissWithIndex:(NSUInteger)buttonIndex
{
    ...
    if (self.blackOpaqueView) {
        [UIView animateWithDuration:timeDisappear animations:^{
            ...
            [self.blackOpaqueView removeFromSuperview]; //注释了下面3行代码消除代理执行两次的情况 YuAng
//            if([self.delegate respondsToSelector:@selector(alertView:didDismissWithButtonIndex:)]){
//                [self.delegate alertView:self didDismissWithButtonIndex:buttonIndex];
//            }
        }];
...
- (void)setupItems
{
    ...
    [self.messageLabel my_setNormalText];
    ...
    [self.cancelButton my_setBtnTextBlue];
...
//    [self.cancelButton addTarget:self action:@selector(cancelButtonTouchEnded:) forControlEvents:UIControlEventTouchDragOutside];
    ...
    [self.otherButton my_setBtnTextBlue];
...
- (void)setupViews
{
    ...
    if (self.separatorColor) {
        ...
        [self.horizontalSeparator my_setSepGrayBg];
        [self.verticalSeparator my_setSepGrayBg];
    }
